...
import java.util.stream.Stream;
 ...
 //Stream API
    /*These streams are related to Collection Framework/(Group of Objects). 
     * These streams are very different from io stream, io streams are sequence of data.
    ...
    /* When we have to represent data or objects collectively(as a single entity) 
     * then we can use Collections but if we have to process data from a collection then we should use stream.
     * 
     * How is stream represented : - 
     * Stream s = collection.stream(); the return type of this method is stream object.
     * Stream is available in java.util.stream package
     * 
     * Important functions used:- 
     * stream(), filter(), map(), collect(),Collectors.toList(), count(), sorted(){default,customized}, max(), min(), Consumer, forEach(), Stream.of(),  */
